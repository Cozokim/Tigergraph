...
class SimpleGradioMetro:
    ...
    def __init__(self):
        ...
        self.path_df_precargado = 'Pickle/SuperSimplified/df_precargado'
        self.path_df_test_precargado = 'Pickle/SuperSimplified/df_test_precargado'

        self.df_precargado = pd.read_csv(self.path_df_precargado)
        self.df_test_precargado= pd.read_csv(self.path_df_test_precargado)

        self.df_columns_precargado = sorted([col for col in self.df_precargado.columns])
        self.df_test_columns_precargado = sorted([col for col in self.df_test_precargado.columns])

        self.model_direction = 'Pickle/supersimplified_best_model'
        self.preprocessing_path = 'Pickle/supersimplified_preprocessing_pipeline'

        self.gemelo_digital_path = 'Pickle/supersimplied_gemelo_digital'
    ...
    def read_csv_path(self, input_files):
        ...
        self.df = pd.read_csv(input_files.name) #if input_files else self.df
        target_column = sorted(list(self.df.columns))

        return gr.Dropdown(choices = target_column)

    def read_csv_test_path(self, input_files = None):

        self.df_test = pd.read_csv(input_files.name) #if input_files else self.df_test
    ...
    def front_func(self):
        ...
        with gr.Blocks(css="Gradio/stylessupersimple.css", title="SUPER CLASSIFIER") as demo:
            gr.Markdown("## ""![](file/Gradio/logo_lis.svg) SUPER CLASSIFIER""", elem_classes="cabecero")
            gr.Markdown("""Solución para la detección de anomalias y predicciones mediante clasificación""", elem_classes="cabecero")
            with gr.Tabs() as tabs:

                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #    
                #                                    Introducción                                                 #
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #   
                with gr.TabItem("Información",id=0):
                    gr.Markdown(

                        """
                        # Elige tu perfil de usuario

                    """, elem_classes="cabecero"
                    )
                    with gr.Row():
                        button_Datascience =  gr.Button('Data Science', elem_classes='button2')
                        def change_tab_datascience():
                            return gr.Tabs(selected=1)
                        button_Datascience.click(change_tab_datascience,None,tabs)
                        
                        button_Predicciones = gr.Button('IT Predicciones', elem_classes='button2')
                        def change_tab_predicciones():
                            return gr.Tabs(selected=2)
                        button_Predicciones.click(change_tab_predicciones,None,tabs)
                        
                        button_gemelo_digital = gr.Button('Manager Gemelo Digital', elem_classes='button2')
                        def change_tab_gemelo():
                            return gr.Tabs(selected=5)
                        button_gemelo_digital.click(change_tab_gemelo,None,tabs)
                ...
                with gr.TabItem("Data Science (Preparacion de Datos y Modelos)",id=1):
                    title_EDA = gr.Markdown("## Exploración de Datos y Entrenamiento", elem_classes='cabecero')
                    
                    with gr.Row(elem_classes="tryrow"):
                        button_trydatascience =  gr.Button('Lanzar un procesado y entrenamiento demostrativos', elem_classes='trybutton')

                    with gr.Row():
                        upload_file = gr.inputs.File(label='Subir el Dataset')

                        with gr.Column():
                            target_column_dropdown = gr.Dropdown(label = 'Seleccione la variable a predecir', choices = self.df_columns_precargado,  interactive=True, allow_custom_value= True, elem_classes="dropdown")
                            ...
                            button_train_models = gr.Button(value = 'Subir Datos y Lanzar Entrenamiento', elem_classes="otherbutton")
                        upload_file.upload(self.read_csv_path, inputs = upload_file, outputs = [target_column_dropdown])

                    def upload_data(target_column_dropdown= None, metric_dropdown = 'accuracy'):

                        if self.df is not None:
                            self.df.to_csv(self.path_df_precargado, index = None)

                        df = self.df if self.df is not None else self.df_precargado
                        self.target = target_column_dropdown 
                        self.metric = metric_dropdown

                        lis_score = LIS_Score(df = df, target = self.target)
                        df_lis_score = round(lis_score.calculate_grades_score())

                        preproc = PreprocessingPipeline(df = df,
                                                        target = self.target,
                        ...
                        return gr.Dataframe(df, visible = True),  training_text, update_EDA, df_lis_score
                    ...
                    with gr.Row():
                        with gr.Column():
                            text_entrenamiento = gr.Textbox(label='Resultados Entrenamiento', value ='Lanzar el Entrenamiento 😀', elem_classes='textbox', info= "Aquí verás el resultado de tu entrenamiento y el mejor modelo para tu variable y metrica objetivo")
                        with gr.Column():
                            lis_score = gr.Textbox(label = 'LIS Score', elem_classes= 'textbox', info = 'Métrica que evalúa la calidad de los datos utilizados en relación con varios criterios. Valor máximo: 100 Valor mínimo: 0')

                    with gr.Row():
                        dataframe = gr.Dataframe(lablel = 'Dataset de Entrenamiento', elem_classes= 'textbox', visible= False)

                    with gr.Row():
                        updatea = gr.Button("Next Tab", elem_classes="button", visible=False)
                        
                        button_train_models.click(upload_data, inputs = [target_column_dropdown, metric_dropdown], outputs = [dataframe, text_entrenamiento,updatea, lis_score])
                ...
                with gr.TabItem("IT (Lanzar Inferencia y Generar Predicciones)",id=2):
                    title_EDA = gr.Markdown("## Predicciones", elem_classes='cabecero')

                    with gr.Row(elem_classes="tryrow"):  
                        button_trypredictions =  gr.Button('Lanzar una predicción demostrativa', elem_classes="trybutton")
                    ...
                    with gr.Row():
                        def boton_inferencia_params(unique_ids, probabilistic_results):
                            ...
                            if self.df_test is not None:
                                self.df.to_csv(self.path_df_test_precargado, index = None)

                            df_test = self.df_test if self.df_test is not None else self.df_test_precargado
                            ...
                            df_test_processed = inference.inference_preprocessing(df_test = df_test)
                            results_proba = True if probabilistic_results == 'True' else False

                            predictions = inference.inference_predictions(df_test = df_test_processed, results_proba = results_proba)                        
                            return gr.Dataframe(predictions, elem_classes='textbox', visible=True), update_inferencia
                        ...
                        with gr.Column(scale=1):
                                unique_ids = gr.Dropdown(label='Columna identificatoria', value = 'None', choices= self.df_test_columns_precargado, multiselect=True, interactive=True, elem_classes='dropdown')
                                ...
                                boton_inference = gr.Button("Subir datos y Lanzar Predicciones", elem_classes="otherbutton")

                    with gr.Row():
                       with gr.Column(scale=1):
                           #boton_inference = gr.Button("Subir datos y Lanzar Predicciones", elem_classes="button") 
                ...
                with gr.TabItem("Manager (Gemelo Digital y Simulación de Escenarios)",id=5):  
                    title_gemelo_digital = gr.Markdown("## Gemelo Digital", elem_classes = "cabecero")
                
                    with gr.Row(elem_classes="tryrow"):
                        boton_gemelo = gr.Button("Actualizar Columnas", elem_classes="button")
                        ...
                        with gr.Column(scale=1):
                            ...
                            def resultados_real_time(*inputs_btn_gemelo):
                                ...
                                try:
                                    ...
                                    inference = InferencePipeline(self.model_direction, self.preprocessing_path, unique_ids=self.unique_ids_col)
